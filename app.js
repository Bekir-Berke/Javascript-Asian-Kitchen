...
const createButton = () => {
  ...
  allButton.addEventListener("click", getAllFoods);
  koreaButton.addEventListener("click", getKoreanFoods);
  japanButton.addEventListener("click", getJapanFoods);
  chinaButton.addEventListener("click", getChinaFoods);
}
const getAllFoods = () => {
  let allMenu = "";
  menu.map(food => {
...
const getKoreanFoods = () => {
  let koreanMenu = "";
  menu.map(food => {
    if(food.category == "Korea"){
      koreanMenu += createMenu(food);
    }
  })
  menuDOM.innerHTML = koreanMenu;
}
